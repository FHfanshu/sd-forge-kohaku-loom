...
## 2026-07-19 Prompt Agent Baseline And Runtime Cutover

- Frozen the final archived runtime at commit `b016c88`, branch `kt`, and tag
  `kt-final`; `docs/current-architecture-audit.md` and
  `docs/kt-runtime-migration.md` retain the detailed baseline and rollback
  contract.
- Pinned Pi Agent Core and Pi AI `0.74.2` with TypeBox `1.1.24` for Node
  `22.17.0`, introduced the frontend Pi controller, Python Prompt Agent API,
  IndexedDB sessions, proxy streaming, profile redaction, and failure-safe
  composer cleanup.
- The original full command output and detailed per-file narrative remain in
  Git history. The verified cutover passed Python, Svelte, frontend coverage,
  Vite build, bundle budget, Playwright, browser syntax, and diff checks.
...
The final archived-runtime entries for this date moved to
`docs/audit-archive-2026-07-19.md` during Phase 9 cleanup.

## 2026-07-19 Archived Runtime Contract Cleanup

- Visible symptom: after the archived sidecar files were removed, the active
  composer, profile settings, and browser fixtures still referenced queue,
  branch, legacy-session, host-owned profile, and sidecar synchronization
  contracts. The removed creature prompt and oversized audit log also broke
  Python architecture checks.
- Root cause: Phase 9 file deletion preceded the final contract and fixture
  migration. The frontend surface still rendered controls for operations that
  no longer had a runtime implementation.
- Changed `frontend/src/contracts.ts`, `frontend/src/stores/chat.ts`,
  `frontend/src/stores/runtime.ts`, and `frontend/src/components/Surface.svelte`
  to remove queue persistence, branch metadata, edit/regenerate actions, legacy
  history sources, and the queue composer control. An active request now exposes
  only Stop while preserving a follow-up draft in the composer.
- Added `frontend/src/profile-api.ts` and moved `frontend/src/stores/profiles.ts`
  and `frontend/src/components/ProfileSettings.svelte` to the server-owned
  `/prompt-agent/api/profiles` and `/profile-routes/default` endpoints. API key
  drafts remain component-local until explicit save; the host namespace no
  longer receives profile or secret operations.
- Updated frontend and browser fixtures to use Prompt Agent profile/stream
  responses, removed obsolete sidecar host-bridge tests, and moved the final
  archived-runtime audit entries to `docs/audit-archive-2026-07-19.md`.
- Changed `tests/test_prompts.py` to assert the active prompt contract without
  reading the deleted creature prompt.
- Verification: `python -m unittest discover -s tests` passed 94 tests;
  `python -m compileall -q backend kohaku_loom scripts install.py tools`
  passed; Node syntax checks passed for both active browser scripts;
  Svelte check passed with 0 errors and 0 warnings; frontend coverage passed
  93 tests across 17 files at 84.48% statements; the Vite build passed and
  regenerated `javascript/kohaku_loom_90_ui.js` at 730,059 raw / 209,524 gzip
  bytes; bundle-size passed; Playwright passed all 6 scenarios.
- Residual risk: README, package identifiers, and several historical audit or
  migration references still use the archived Kohaku Loom name and require the
  separate Phase 10 naming and licensing decision.

## 2026-07-19 Profile Restore Persistence

- Root cause: the new profile store implemented Restore defaults as a local
  browser reset even though Python is the profile authority, so a refresh could
  restore the previous server state.
- Added `POST /prompt-agent/api/profiles/restore-defaults`, server-side secret
  clearing, and frontend persistence through `profile-api.ts`.
- Added API coverage for CRUD, duplicate/delete, default routing, secret-free
  projections, restore-defaults, and the default storage path's exclusion of
  `.loom`.
- Verification: focused Python API, architecture, prompt, and host tests passed
  32 tests. Final repository gates passed 96 Python tests, Python compileall,
  Svelte check with 0 errors and 0 warnings, 93 frontend tests across 17 files
  at 84.38% statement coverage, the Vite build, bundle-size at 730,169 raw /
  209,543 gzip bytes, all 6 Playwright scenarios, and syntax checks for the
  active and generated browser scripts.

## 2026-07-19 Phase 9 Runtime And Profile Privacy Cleanup

- Root causes: the boot contract fixture did not model the generated gate's
  readiness event listener; the architecture suite still protected the deleted
  `kohaku_loom.generic` facade; public Profile responses echoed local GGUF,
  mmproj, and llama-server paths; and `kohaku_loom/constants.py` still carried
  the deleted Python assistant system prompt and OpenAI-style tool schema.
- Changed `backend/prompt_agent/profile_contracts.py` so local paths remain
  Python-owned and public profiles expose only `localModelConfigured`,
  `mmprojConfigured`, and `llamaServerConfigured`. Updated the frontend Profile
  contract and settings UI to keep newly entered paths in component-local,
  write-only drafts until explicit Save; saved paths are not returned to or
  retained in browser state.
- Removed the obsolete generic-facade architecture assertion and old Python
  assistant tool-contract test. Reduced `kohaku_loom/constants.py` to constants
  still used by local model discovery and reference-image analysis. Updated the
  boot fixture for the real generated readiness contract and regenerated
  `javascript/kohaku_loom_90_ui.js`.
- Regression coverage verifies ProfileAuthority redacts all local paths while
  preserving configuration flags, the Svelte profile store never receives a
  typed local path, explicit Save submits the write-only draft, and the browser
  boot gate still mounts only through Forge's UI callback.
- Verification: `python -m compileall -q backend kohaku_loom scripts install.py
  tools` passed; `python -m unittest discover -s tests` passed 40 tests; all 11
  standalone browser contract tests passed; Svelte check reported 0 errors and
  0 warnings; frontend coverage passed 95 tests across 17 files at 84.42%
  statements; Playwright passed all 6 desktop/mobile/tablet scenarios; Vite
  transformed 4,779 modules and regenerated the bundle at 731,008 raw / 209,761
  gzip bytes, below budget; browser syntax checks and `git diff --check` passed
  with only existing npm `store-dir` and Git line-ending warnings.
