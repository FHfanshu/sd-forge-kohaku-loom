...
def _select_llama_cpp_windows_asset() -> dict[str, str]:
    req = urllib.request.Request(LLAMA_CPP_RELEASE_API, headers={"User-Agent": "sd-forge-neo-prompt-agent"})
...
def _download_url(url: str, target: Path) -> None:
    ...
    if tmp.exists():
        tmp.unlink()
    req = urllib.request.Request(url, headers={"User-Agent": "sd-forge-neo-prompt-agent"})
