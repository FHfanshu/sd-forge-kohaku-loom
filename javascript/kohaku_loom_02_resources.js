(function () {
    ...
    async function searchResourcesTool(args, signal) {
        return await resourceGet("/kohaku-loom/resources/search", {
            kind: args.kind,
            query: args.query || "",
            limit: args.limit || 20,
            cursor: args.cursor || ""
    ...
    async function inspectResourceTool(args, signal) {
        return await resourceGet("/kohaku-loom/resources/inspect", {
            kind: args.kind,
            id: args.id,
            query: args.query || "",
            limit: args.limit || 20,
    ...
    async function loadPromptSkillTool(args, signal) {
        ...
        const result = await resourceGet(`/kohaku-loom/prompt-skills/${encodeURIComponent(name)}`, {}, signal);
    ...
    async function searchDanbooruTagsTool(args, signal) {
        ...
        return await resourceGet("/kohaku-loom/danbooru/tags/search", {
            query: args.query || "",
            queries: queries,
    ...
    async function inspectDanbooruTagTool(args, signal) {
        return await resourceGet("/kohaku-loom/danbooru/tags/inspect", { name: args.name || "" }, signal);
    }

    async function inspectDanbooruTagsTool(args, signal) {
        const names = Array.isArray(args.names) ? args.names.join(",") : "";
        return await resourceGet("/kohaku-loom/danbooru/tags/inspect-batch", { names: names, include_wiki: !!args.include_wiki }, signal);
    }

    async function relatedDanbooruTagsTool(args, signal) {
        return await resourceGet("/kohaku-loom/danbooru/tags/related", {
            name: args.name || "",
            category: args.category || "",
            limit: args.limit || 12
        }, signal);
    ...
    async function addStyleSelection(target, styleName) {
        const root = loomApp().querySelector(`#${target}_styles`);
        ...
        if (!String(updated || "").toLowerCase().includes(normalized)) {
            const option = Array.from(loomApp().querySelectorAll("[role='option'], [data-testid='option']")).find(function (node) {
