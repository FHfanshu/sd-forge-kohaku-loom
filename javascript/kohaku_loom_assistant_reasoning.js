(function () {
    ...
    function persistEffort(value) {
        ...
        window.dispatchEvent?.(new CustomEvent("loom:model-profiles-changed", { detail: tools.profileStore.load() }));
    }

    function buildPopover(button) {
        const popover = document.createElement("div");
        popover.id = "loom_assistant_reasoning_popover";
        popover.className = "loom-assistant-reasoning-popover";
        popover.hidden = true;
        popover.setAttribute("role", "dialog");
        popover.setAttribute("aria-label", t("settings.reasoning_effort", "推理强度"));
        popover.innerHTML = `
            <div class="loom-reasoning-heading"><span>${t("settings.reasoning_effort", "推理强度")}</span><strong data-loom-effort-label></strong></div>
            <label class="loom-reasoning-toggle"><span>${t("settings.reasoning_enable", "启用思考")}</span><input type="checkbox"><i aria-hidden="true"></i></label>
            <div class="loom-reasoning-scale-labels"><span>${t("settings.reasoning_faster", "更快")}</span><span>${t("settings.reasoning_smarter", "更聪明")}</span></div>
            <div class="loom-reasoning-range-wrap"><div class="loom-reasoning-dots" aria-hidden="true"></div><input type="range" min="0" max="3" step="1" value="0" aria-label="${t("settings.reasoning_effort", "推理强度")}"></div>`;
        ...
        toggle.addEventListener("change", function () {
            ...
            const remembered = button.dataset.loomLastEffort;
        ...
        range.addEventListener("input", function () {
            ...
            popover.querySelector("[data-loom-effort-label]").textContent = effortLabel(value);
        });
    ...
    function syncAssistantReasoningControl() {
        const button = document.querySelector("#loom_assistant_reasoning");
        if (!button) return;
        const profile = activeProfile();
        ...
        if (effort !== "none") button.dataset.loomLastEffort = effort;
        button.dataset.loomEffort = effort;
        ...
        const popover = document.querySelector("#loom_assistant_reasoning_popover");
        if (!popover) return;
        const toggle = popover.querySelector('input[type="checkbox"]');
        const range = popover.querySelector('input[type="range"]');
        const currentIndex = Math.max(0, levels.indexOf(effort === "none" ? button.dataset.loomLastEffort : effort));
        ...
        popover.querySelector("[data-loom-effort-label]").textContent = effortLabel(effort);
        const dots = popover.querySelector(".loom-reasoning-dots");
    ...
    function setupAssistantReasoningControl(panel) {
        const button = panel?.querySelector("#loom_assistant_reasoning");
        if (!button || button.dataset.loomReasoningBound === "1") return;
        button.dataset.loomReasoningBound = "1";
